            ...
            <div class="container pb-5">
                ...
                <div class="row row-cols-2 row-cols-sm-3 row-cols-lg-6 g-4">
                    ...
                    @foreach ($referral_links as $referral_link)
                        @if ($referral_link->category_id == $cat->id)
                            @if ($count < 6)
                                <!-- Item -->
                                <a href="{{ url('terms_condition/' . $referral_link->id) }}"
                                    class="col text-decoration-none">
                                    <div class="rounded p-4 banking-box">
                                        <img src="{{ asset('images/' . $referral_link->logo) }}"
                                            alt="{{ $referral_link->display_name }}" />
                                        <h5 class="pt-3">{{ $referral_link->display_name }}</h5>
                                        <h6>${{ $referral_link->expected_payout }}</h6>
                                    </div>
                                </a>
                                @php
                                    $count++;
...
<section class="pt-5 pb-5 reward-section">
    <div class="container pb-5">
        ...
        <div class="d-flex pb-4 reward-data">
            ...
            <button class="button-primary" type="submit" id="button-addon2">
                Search
            </button>
        </div>
    </div>
